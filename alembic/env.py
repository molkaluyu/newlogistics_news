"""Alembic async migration environment for logistics_news.

Uses asyncpg via SQLAlchemy's async engine to run migrations against
the same database the application connects to.
"""
...
# so we always migrate against the correct database.
...
def do_run_migrations(connection: Connection) -> None:
    """Callback executed inside the async connection context."""
    context.configure(
